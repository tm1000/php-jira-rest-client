...
class IssueTest extends TestCase
{
	...
	public function create_issue(): string
	{
		try {
			...
			$due = (new DateTime('NOW'))->add(
				DateInterval::createFromDateString('1 month 5 day'),
			);
	 ...
	 */
	public function add_attachment_on_subtask_issue(
		string $subTaskIssueKey,
	): string {
		try {
			$files = [
	...
	public function add_comments(string $subTaskIssueKey): string
	{
		...
		try {
			...
			** sub Bullet 2
			...
			$comment->setBody($body)->setVisibilityAsString('role', 'Users');
	...
	{
		$jql =
			'project not in (TEST)  and assignee = currentUser() and status in (Resolved, closed)';
		try {
			$issueService = new IssueService();

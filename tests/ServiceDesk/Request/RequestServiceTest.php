...
class RequestServiceTest extends TestCase
{
	...
	public function testGet(): void
	{
		...
		self::assertSame(
			$item['requestFieldValues'],
			$result->requestFieldValues,
		);
	}
	...
	public function testGetRequestsByCustomer(): void
	{
		...
		$result = $this->uut->getRequestsByCustomer(
			$customer,
			[],
			$this->serviceDeskId,
		);
	...
	public function testAddAttachments(): void
	{
		...
		$fileNames = ['/uploads/file_1.png', '/uploads/file_2.png'];
	...
	public function testGetTimeTracking(): void
	{
		...
		self::assertSame(
			$field->timeTracking->getTimeSpentSeconds(),
	...
	public function testRemoveWatcher(): void
	{
		...
		$this->client
			->method('exec')
			->with(
				$this->uri . "/20/watchers/?username=$watcher",
				'',
				'DELETE',
			);
		$this->client->method('getHttpResponse')->willReturn(204);
	...
	public function testGetEditMeta(): void
	{
		...
		$url =
			...
			'/' .
			$key .
			'/editmeta?overrideEditableFlag=0&overrideScreenSecurity=0';
	...
	public function testUpdateLabels(): void
	{
		...
		$this->client
			...
			->method('exec')
			->with(
				$this->uri . '/20?notifyUsers=1',
				json_encode(['update' => $labels]),
				'PUT',
			)
			->willReturn(true);

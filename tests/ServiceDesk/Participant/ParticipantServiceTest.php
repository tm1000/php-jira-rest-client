...
class ParticipantServiceTest extends TestCase
{
    ...
    public function testGetParticipant(): void
    {
        ...
        $this->client
            ->method('exec')
            ->willReturn(json_encode(['values' => [$customerData]]));
    ...
    public function testAddParticipant(): void
    {
        ...
        $result = $this->uut->addParticipantToRequest('K-123', [
            $this->createCustomer($customerData[1]),
        ]);

        $this->assertEquals(
            [
                $this->createCustomer($customerData[0]),
                $this->createCustomer($customerData[1]),
            ],
            $result
        );
    ...
    public function testRemoveParticipant(): void
    {
        ...
        $result = $this->uut->removeParticipantFromRequest('K-123', [
            $this->createCustomer(['accountId' => 'la-la']),
        ]);

...
class ProjectService extends \JiraRestApi\JiraClient
{
	 ...
	 * @return Project
	 */
	public function getProjectFromJSON($json): Project
	{
		$project = $this->json_mapper->map($json, new Project());
	...
	public function getAssignable(int|string $projectIdOrKey): array
	{
		...
		$results = array_map(fn($elem) => $this->json_mapper->map(
			$elem,
			new Reporter(),
		), $json);
	...
	public function getProjectType(int|string $key): ProjectType
	{
		...
		$type = $this->json_mapper->map(json_decode($ret, false), new ProjectType());
	...
	public function getVersionsPagenated(
		...
		$param = $this->toHttpQueryParameter(array_merge($default, $queryParam));

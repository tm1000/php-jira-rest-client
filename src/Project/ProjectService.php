...
class ProjectService extends \JiraRestApi\JiraClient
{
	...
	{
		$ret = $this->exec(
			"/user/assignable/search?project=$projectIdOrKey",
			null,
		);
		$json = json_decode($ret);
		$results = array_map(
			fn($elem) => $this->json_mapper->map($elem, new Reporter()),
			$json,
		);
	...
	public function getStatuses(int|string $projectIdOrKey): array
	{
		...
		$json = json_decode($ret);
		$results = array_map(
			fn($elem) => $this->json_mapper->map($elem, new IssueType()),
			$json,
		);
	...
	public function getProjectComponents(int|string $projectIdOrKey): array
	{
		...
		$json = json_decode($ret);
		$results = array_map(
			fn($elem) => $this->json_mapper->map($elem, new Component()),
			$json,
		);
	 ...
	 * @return array
	 */
	public function getProjectTransitionsToArray(
		int|string $projectIdOrKey,
	): array {
		$ret = $this->exec($this->uri . "/$projectIdOrKey/statuses", null);
		$json = json_decode($ret);
		$results = array_map(
			...
			foreach ($issueType->statuses as $status) {
				if (!in_array($status->id, array_column($transitions, 'id'))) {
					$transitions[] = [
						...
						'name' => $status->name,
						'untranslatedName' =>
							$status->untranslatedName ?? $status->name,
					];
				}
			}
		}
	...
	public function getProjectTypes(): array
	{
		...
		$json = json_decode($ret);
		$results = array_map(
			fn($elem) => $this->json_mapper->map($elem, new ProjectType()),
			$json,
		);
	 ...
	 */
	public function getVersionsPagenated(
		int|string $projectIdOrKey,
		array $queryParam = [],
	): \ArrayObject {
		$default = [
		...
		$versions = $this->json_mapper->mapArray(
			$json->values,
			new \ArrayObject(),
			\JiraRestApi\Issue\Version::class,
		);
	...
	public function getVersion(
		...
		throw new JiraException(
			"Can't found version \"$versionName\" in the Project \"$projectIdOrKey\"",
		);
	}
	...
	public function createProject(Project $project): Project
	{
		...
		return $this->json_mapper->map(json_decode($ret), new Project());
	}
	...
	{
		$ret = $this->exec(
			$this->uri . '/' . $projectIdOrKey . '/archive',
			null,
			'PUT',
		);
	 ...
	 * @return string
	 */
	public function assignRoleToProject(
		int|string $projectIdOrKey,
		int $roleId,
	): string {
		return $this->exec(
			$this->uri . '/' . $projectIdOrKey . '/role/' . $roleId,
			null,
			'PUT',
		);
	}
	 ...
	 * @return string
	 */
	public function addProjectRoleActors(
		int|string $projectIdOrKey,
		int $roleId,
		string $actor,
	): string {

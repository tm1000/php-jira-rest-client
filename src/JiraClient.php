...
class JiraClient
{
	...
	public function __construct(
		...
		if (
			...
			$this->httpClient = $httpClient ?? new \GuzzleHttp\Client($guzzleConfig);
			$this->requestFactory =
	...
	protected function filterNullVariable(array $haystack): array
	{
		foreach ($haystack as $key => $value) {
			if (is_array($value)) {
				...
				$haystack[$key] = $this->filterNullVariable(get_class_vars(get_class(
					$value,
				)));
			}
	...
	protected function sendRequest(
		...
		try {
			...
			throw new JiraException('HTTP Request Failed: ' . $e->getMessage(), 0, $e);
		}
	}
	...
	public function exec(
		...
		if (is_string($post_data)) {
			...
			$this->log->info(
				"$custom_request $url: " . json_encode($post_data, JSON_UNESCAPED_UNICODE),
			);
		}
		...
		try {
			...
			throw new JiraException('Request failed: ' . $e->getMessage(), 0, $e);
		}
	}
	...
	public function upload(string $context, array $filePathArray): array
	{
		...
		foreach ($filePathArray as $idx => $filePath) {
			if (!file_exists($filePath) || !is_readable($filePath)) {
				throw new JiraException("File not found or not readable: $filePath");
			}
			...
			try {
				...
				// Make the request
				$response = $this->sendRequest('POST', $url, $headers, $multipartStream);
	...
	public function toHttpQueryParameter(
		...
		foreach ($paramArray as $key => $value) {
			...
			$queryParam .= rawurlencode($key) . '=' . rawurlencode((string) $v) . '&';
		}
	 ...
	 */
	public function download(string $url, string $outDir, string $filename): string
	{
		...
		try {
			...
			if ($statusCode < 200 || $statusCode >= 300) {
				throw new JiraException(
					'Download failed with status code: ' . $statusCode . ', URL: ' . $url,
					(int) $statusCode,
				);
			}
			...
			throw new JiraException('Download failed: ' . $e->getMessage(), 0, $e);
		}
	}

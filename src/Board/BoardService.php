...
class BoardService extends \JiraRestApi\JiraClient
{
	...
	public function getBoardList($paramArray = []): ?\ArrayObject
	{
		...
		try {
			...
			$this->log->error(
				"Response cannot be decoded from json\nException: {$exception->getMessage()}",
			);
	...
	{
		$json = $this->exec(
			$this->uri . '/' . $id . $this->toHttpQueryParameter($paramArray),
			null,
		);
	 ...
	 * @return \ArrayObject|AgileIssue[]|null
	 */
	public function getBoardBacklogIssues(
		$id,
		array $paramArray = [],
	): ?\ArrayObject {
		$json = $this->exec(
	 ...
	 * @return PaginatedResult|null array of Board class
	 */
	public function getSprintsForBoard(
		$boardId,
		$paramArray = [],
	): ?PaginatedResult {
		$json = $this->exec(

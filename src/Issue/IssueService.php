...
class IssueService extends \JiraRestApi\JiraClient
{
	...
	{
		$issue = $this->json_mapper->map($json, new Issue());
	...
	public function get(
		...
		$issueObject = $issueObject ?: new Issue();
	...
	public function addAttachments($issueIdOrKey, $filePathArray): array
	{
		...
		$results = $this->upload(
			$this->uri . "/$issueIdOrKey/attachments",
			$filePathArray,
		);
		...
		foreach ($results as $ret) {
			...
			if (is_array($ret)) {
				...
				$attachArr[] = $this->json_mapper->map($ret, new Attachment());
			}
		}
	...
	public function update($issueIdOrKey, $issueField, $paramArray = []): string
	{
		...
		$ret = $this->exec(
			$this->uri . "/$issueIdOrKey" . $queryParam,
			$data,
			'PUT',
		);
	...
	public function addComment($issueIdOrKey, $comment): Comment
	{
		...
		if (!($comment instanceof Comment) || empty($comment->body)) {
			throw new JiraException(
				'comment param must be instance of Comment and have body text.',
			);
		}
		...
		$comment = $this->json_mapper->map(json_decode($ret), new Comment());
	...
	public function updateComment($issueIdOrKey, $id, $comment): Comment
	{
		...
		if (!($comment instanceof Comment) || empty($comment->body)) {
			throw new JiraException(
				'comment param must instance of Comment and have to body text.!',
			);
		}
		...
		$ret = $this->exec(
			$this->uri . "/$issueIdOrKey/comment/$id",
			$data,
			'PUT',
		);
	...
	public function getComments($issueIdOrKey, array $paramArray = []): Comments
	{
		...
		$comments = $this->json_mapper->map(json_decode($ret), new Comments());
	...
	public function changeAssignee($issueIdOrKey, $assigneeName): string|bool
	{
		...
		$ret = $this->exec(
			$this->uri . "/$issueIdOrKey/assignee",
			$data,
			'PUT',
		);
	...
	public function getTransition($issueIdOrKey, $paramArray = []): ArrayObject
	{
		...
		$ret = $this->exec(
			$this->uri . "/$issueIdOrKey/transitions" . $queryParam,
		);
	...
	public function findTransitonId($issueIdOrKey, $transitionToName): string
	{
		...
		foreach ($ret as $trans) {
			...
			$this->log->debug(
				'getTransitions result=' . var_export($ret, true),
			);

			if (strcasecmp((string) $toName, $transitionToName) === 0) {
				return $trans->id;
			}
		}
		...
		throw new JiraException(
			"Transition name '$transitionToName' not found on JIRA Server.",
		);
	}
	...
	public function transition($issueIdOrKey, $transition): ?string
	{
		...
		if (!isset($transition->transition['id'])) {
			if (isset($transition->transition['untranslatedName'])) {
				$transition->transition[
					'id'
				] = $this->findTransitonIdByUntranslatedName(
					$issueIdOrKey,
					$transition->transition['untranslatedName'],
				);
			} elseif (isset($transition->transition['name'])) {
				$transition->transition['id'] = $this->findTransitonId(
					$issueIdOrKey,
					$transition->transition['name'],
				);
			} else {
				throw new JiraException(
					'you must set either name or untranslatedName for performing transition.',
				);
			}
		}
		...
		$ret = $this->exec(
			$this->uri . "/$issueIdOrKey/transitions",
			$data,
			'POST',
		);
	...
	public function search(
		...
		$result = $this->json_mapper->map($json, new IssueSearchResult());
	...
	public function getTimeTracking(string $issueIdOrKey): TimeTracking
	{
		...
		$issue = $this->json_mapper->map(json_decode($ret), new Issue());
	...
	public function timeTracking(
		...
		$array = [
			'update' => [
				'timetracking' => [['edit' => $timeTracking]],
			],
		];
	...
	{
		$ret = $this->exec(
			$this->uri . "/$issueIdOrKey/properties/" . $property,
		);

		$this->log->debug("getProperty res=$ret\n");

		return $this->json_mapper->map(json_decode($ret), new Property());
	}
	 ...
	 */
	public function getWorklogById(
		string $issueIdOrKey,
		int $workLogId,
	): Worklog {
		...
		return $this->json_mapper->map(json_decode($ret), new Worklog());
	}
	...
	public function getWorklogsByIds(array $ids): array
	{
		...
		$worklogs = array_map(
			fn($worklog) => $this->json_mapper->map($worklog, new Worklog()),
			$worklogsResponse,
		);
	...
	public function getPriority(int $priorityId): Priority
	{
		...
		return $this->json_mapper->map(json_decode($ret), new Priority());
	}
	...
	public function getCustomFields(
		...
		$ret = $this->exec(
			'customFields' . $this->toHttpQueryParameter($paramArray),
			null,
		);
	...
	public function removeWatcher(string $issueIdOrKey, string $watcher): bool
	{
		...
		$ret = $this->exec(
			$this->uri . "/$issueIdOrKey/watchers/?username=$watcher",
			'',
			'DELETE',
		);
	 ...
	 * @return bool
	 */
	public function removeWatcherByAccountId(
		string $issueIdOrKey,
		string $accountId,
	): bool {
		$this->log->debug("removeWatcher=\n");
		...
		$ret = $this->exec(
			$this->uri . "/$issueIdOrKey/watchers/?accountId=$accountId",
			'',
			'DELETE',
		);
	 ...
	 * @return string|bool
	 */
	public function removeRemoteIssueLink(
		string $issueIdOrKey,
		string $globalId,
	): string|bool {
	...
	public function getAllIssueSecuritySchemes(): ArrayObject
	{
		...
		// extract schem field
		$schemes = json_decode(
			json_encode($data['issueSecuritySchemes']),
			false,
		);
	...
	public function getIssueSecuritySchemes(string $securityId): SecurityScheme
	{
		...
		$res = $this->json_mapper->map(json_decode($ret), new SecurityScheme());
	...
	public function updateLabels(
		string $issueIdOrKey,
		array $addLablesParam = [],
		array $removeLabelsParam = [],
	 ...
	 * @return bool
	 */
	public function updateFixVersions(
		string $issueIdOrKey,
		array $addFixVersionsParam,
		array $removeFixVersionsParam,
		bool $notifyUsers = true,
	): bool {
		$fixVersions = [];
	 ...
	 * @return string
	 */
	public function findTransitonIdByUntranslatedName(
		string $issueIdOrKey,
		...
		$this->log->debug(
			'getTransitions result=' . var_export($transitionArray, true),
		);

		foreach ($transitionArray as $trans) {
			if (
				strcasecmp((string) $trans['name'], $untranslatedName) === 0 ||
				strcasecmp(
					$trans['untranslatedName'] ?? '',
					$untranslatedName,
				) === 0
			) {
				return $trans['id'];
		...
		// transition keyword not found
		throw new JiraException(
			"Transition name '$untranslatedName' not found on JIRA Server.",
		);
	}
}

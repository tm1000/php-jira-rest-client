...
class ParticipantService
{
	...
	public function __construct(private readonly ServiceDeskClient $client)
	{
		...
		$this->mapper = $this->client->getMapper();
	}
	 ...
	 */
	public function getParticipantOfRequest(
		string $issueIdOrKey,
		int $start = 0,
		int $limit = 50,
	): array {
		...
		$result = $this->client->exec(
			$this->client->createUrl(
				'/request/%s/participant?start=%d&limit=%d',
				[$issueIdOrKey, $start, $limit],
			),
		);

		$this->logger->debug(
			'get participant result=' . var_export($result, true),
		);
	 ...
	 */
	public function addParticipantToRequest(
		string $issueIdOrKey,
		array $participants,
	): array {
		...
		$result = $this->client->exec(
			$this->client->createUrl('/request/%s/participant', [
				$issueIdOrKey,
			]),
		...
		$this->logger->debug(
			'add participant result=' . var_export($result, true),
		);
	 ...
	 */
	public function removeParticipantFromRequest(
		string $issueIdOrKey,
		array $participants,
	): array {
		...
		$this->logger->debug(
			'remove participant result=' . var_export($result, true),
		);
	...
	private function mapResult(string $result): array
	{
		...
		return array_map(
			fn(object $user): Customer => $this->mapper->map(
				$user,
				new Customer(),
			),
			$userData->values ?? [],

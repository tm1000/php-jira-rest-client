...
class OrganisationService
{
	 ...
	 * @see https://docs.atlassian.com/jira-servicedesk/REST/5.2.0/#servicedeskapi/organization-createOrganization
	 */
	public function createFromOrganisation(
	...
	public function getOrganisations(int $start, int $limit): array
	{
		...
		foreach ($organisationData['values'] as $organisation) {
			$organisations[] = $this->jsonMapper->map(
				$organisation,
				new Organisation(),
			);
		}
	 ...
	 * @return Customer[]
	 */
	public function getCustomersForOrganisation(
		int $startIndex,
		int $amountOfItems,
		Organisation $organisation,
	): array {
		$result = $this->client->exec(
			$this->createGetCustomersUrl(
				$organisation->id,
				$startIndex,
				$amountOfItems,
			),
		...
		foreach ($customerData as $customer) {
			$customers[] = $this->jsonMapper->map($customer, new Customer());
		}
	 ...
	 */
	public function addCustomersToOrganisation(
		array $customers,
		Organisation $organisation,
	): void {
		$customerNames = array_map(
			static fn(Customer $customer) => $customer->name,
			$customers,
		);
	 ...
	 */
	private function createGetCustomersUrl(
		int $organisationId,
		int $startIndex,
		int $amountOfItems,
	): string {

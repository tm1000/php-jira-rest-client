...
class RequestService
{
    ...
    public function __construct(
        private readonly ServiceDeskClient $client,
    ...
    {
        return $this->jsonMapper->map($jsonData, new Request());
    }
     ...
     */
    public function getRequestsByCustomer(
        Customer $customer,
        ...
        foreach ($requestData->values as $request) {
            $requests[] = $this->jsonMapper->map($request, new Request());
        }
    ...
    public function addAttachments(
        ...
        $temporaryFileNames = $this->attachmentService->createTemporaryFiles(
            $attachments,
            $serviceDeskId
        );

        $attachments = $this->attachmentService->addAttachmentToRequest(
            $requestId,
            $temporaryFileNames
        );

        $this->logger->info(
            'addAttachments result=' . var_export($attachments, true)
        );
     ...
     * @throws JiraException|JsonException
     */
    public function changeAssigneeByAccountId(
        string $issueIdOrKey,
        ?string $accountId
    ): string {
     ...
     * @throws JiraException|JsonMapper_Exception|JsonException
     */
    public function findTransitionId(
        string|int $issueIdOrKey,
        string $transitionToName
    ): string {
        ...
        throw new JiraException(
            sprintf(
                "Transition name '%s' not found on JIRA Server.",
                $transitionToName
            )
        );
    }
    ...
    public function transition(
        ...
        if (!isset($transition->transition['id'])) {
            if (isset($transition->transition['untranslatedName'])) {
                $transition->transition[
                    'id'
                ] = $this->findTransitionIdByUntranslatedName(
                    $issueIdOrKey,
                    $transition->transition['untranslatedName']
                );
            } elseif (isset($transition->transition['name'])) {
                $transition->transition['id'] = $this->findTransitionId(
                    $issueIdOrKey,
    ...
    public function getWorklog(
        ...
        $ret = $this->client->exec(
            $this->uri .
                ...
                $this->client->toHttpQueryParameter($paramArray)
        );
    ...
    public function getWorklogById(
        ...
        $ret = $this->client->exec(
            $this->uri . "/$issueIdOrKey/worklog/$workLogId"
        );
    ...
    public function getWorklogsByIds(array $ids): array
    {
        ...
        $worklogs = array_map(
            fn($worklog) => $this->jsonMapper->map($worklog, new Worklog()),
            $worklogsResponse
        );
    ...
    public function getCreateMeta(
        array $paramArray = [],
        bool $retrieveAllFieldsAndValues = true
    ): object {
        $paramArray['expand'] = $retrieveAllFieldsAndValues
    ...
    public function notify(string $issueIdOrKey, Notify $notify): void
    {
        ...
        $notify->restrict['groups'] = $this->notifySetSelf(
            $notify->restrict['groups']
        );
    ...
    public function getAllIssueSecuritySchemes(): array
    {
        ...
        return $this->jsonMapper->mapArray($schemes, [], SecurityScheme::class);
    }
     ...
     * @throws JiraException
     */
    public function findTransitionIdByUntranslatedName(
        string|int $issueIdOrKey,
        ...
        $this->logger->debug(
            'getTransitions result=' . var_export($transitions, true)
        );

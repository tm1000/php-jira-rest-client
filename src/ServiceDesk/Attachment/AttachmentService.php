...
class AttachmentService
{
	...
	public function createTemporaryFiles(
		...
		return $this->client->upload(
			$this->client->createUrl('/servicedesk/%s/attachTemporaryFile', [
				$serviceDeskId,
			]),
			$fileNames,
		);
	 ...
	 * @return Attachment[]
	 */
	public function addAttachmentToRequest(
		int $requestId,
	...
	{
		return array_map(
			static fn(Attachment $attachment) => $attachment->filename,
			$attachments,
		);
	}
	...
	private function createAttachmentsFromJson(string $result): array
	{
		...
		foreach ($attachmentData as $attachment) {
			$attachments[] = $this->jsonMapper->map(
				$attachment,
				new Attachment(),
			);
		}

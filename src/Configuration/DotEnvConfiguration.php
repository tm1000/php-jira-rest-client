...
class DotEnvConfiguration extends AbstractConfiguration
{
	...
	public function __construct(string $path = '.')
	{
		...
		$this->jiraLogFile = $this->env(
			'JIRA_LOG_FILE',
			'jira-rest-client.log',
		);
		$this->jiraLogLevel = $this->env('JIRA_LOG_LEVEL', 'WARNING');
		$this->curlOptSslVerifyHost = $this->env(
			'CURLOPT_SSL_VERIFYHOST',
			false,
		);
		$this->curlOptSslVerifyPeer = $this->env(
		...
		$this->curlOptUserAgent = $this->env(
			'CURLOPT_USERAGENT',
			$this->getDefaultUserAgentString(),
		);
	...
	{
		$requireParam = ['JIRA_HOST', 'TOKEN_BASED_AUTH'];
